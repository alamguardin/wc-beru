...
export class Toast {
	...
	#renderToast(data) {
		...
		if (data.type === 'success') toastIcon = ico.successIcon();
		if (data.type === 'error') toastIcon = ico.errorIcon();
		if (data.type === 'warning') toastIcon = ico.warningIcon();
		if (data.type === 'info') toastIcon = ico.infoIcon();
	 ...
	 * @param {string} [options.beruId] The id of the wc-beru instance to display the toast.
